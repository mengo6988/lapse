/**
 * Wires buildDetailRows + buildVariantInsights to VariantSummaryRow, and
 * owns the one-tap accept: it writes the suggested Threshold back to
 * whichever entity actually owns the effective value being evaluated — the
 * Variant itself for a Variant row (even one currently inheriting the
 * parent's Threshold, since the suggestion is Variant-specific data), or
 * the Tracker for a tracker-level row (a Tracker without Variants).
 * docs/spec.md doesn't say which of the two a suggestion's accept should
 * target when Variants exist — this is the interpretation this ticket
 * settled on; see the ticket notes for the reasoning.
 */
...
export function VariantSummaryList({ tracker, now, loadedEntries }: VariantSummaryListProps) {
  const rows = buildDetailRows(tracker, now)
