 ...
 * src/client/tracker/mutationClient.ts per this ticket's instructions,
 * rather than a second error-parsing path. The response is empty (204), so
 * the cache update only needs the id the caller already had.
 */
  ...
  return useMutation({
    ...
    onSuccess: (_response, trackerId) => {
      queryClient.setQueryData<BootstrapPayload>(bootstrapQueryKey, (old) =>
        old ? removeTrackerFromCache(old, trackerId) : old,
      )
    },
  })
}
